...
import { formatTime, isLowTime, isCriticalTime } from '../../utils/timerModes';
import type { PlayerState, GameModeSettings, PlayerNumber } from '../../types';
...
export const TimerDisplay = ({
  ...
  // 残り時間が少ない場合の警告状態を判定
  const lowTime = isLowTime(player.remainingTime);
  const criticalTime = isCriticalTime(player.remainingTime);
  ...
  const buttonClasses = [
    ...
    criticalTime && 'animate-pulse-critical',
    lowTime && !criticalTime && 'animate-pulse-warning',
  ]
    .filter(Boolean)
    .join(' ');
  ...
  const getAriaLabel = (): string => {
    const time = formatTime(
      ...
      player.isInByoyomi || criticalTime
    );
